...
func (d db) Create(ctx context.Context, user user.User) (string, error) {
	...
	d.logger.Trace(user)
	return "", fmt.Errorf("failed to convert objected to hex")
...
func (d db) FindOne(ctx context.Context, id string) (u user.User, err error) {
	...
	if err != nil {
		return u, fmt.Errorf("failed to convert object ID from hex: %s", id)
	}
	...
	if err = result.Decode(&u); err != nil {
		return u, fmt.Errorf("failed to decode user (id: %s) from DB due ot error: %v", id, err)
	}
...
func (d db) FindAll(ctx context.Context) (u []user.User, err error) {
	...
	if cursor.Err() != nil {
		return u, fmt.Errorf("failed to find one All user due to error: %v", err)
	}
...
func (d db) Delete(ctx context.Context, id string) error {
	...
	d.logger.Tracef("Matched %d documents", result.DeletedCount)
	return nil
}
